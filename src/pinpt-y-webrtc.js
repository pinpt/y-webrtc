 ...
 */
const readMessage = (room, peerUID, buf, syncedCallback) => {
  if (room === undefined) {
    return null
  }
 ...
 * @return {encoding.Encoder?}
 */
const readPeerMessage = (peerConn, buf, peerUID) => {
  ...
  return readMessage(room, peerUID, buf, () => {
...
const broadcastWebrtcConn = (room, m) => {
  ...
  room.webrtcConns.forEach((conn) => {
    try {
      ...
      log(
        'error sending broadcast message to ',
        logging.BOLD,
        room.name,
        logging.UNBOLD,
        ': ',
...
export class WebrtcConn {
  ...
  constructor (signalingConn, initiator, remotePeerId, remoteUID, room) {
    ...
    this.peer.on('data', (data) => {
      const answer = readPeerMessage(this, data, this.remoteUID)
      if (answer !== null) {
...
const announceSignalingInfo = (room) => {
  signalingConns.forEach((conn) => {
    ...
    if (conn.connected) {
      ...
      log(
        'announceSignalingInfo but was not connected ',
        logging.BOLD,
        room.name,
        logging.UNBOLD
      )
  ...
  constructor (doc, provider, name, key, uid) {
    ...
    this._bcSubscriber = (data) =>
      cryptoutils.decrypt(new Uint8Array(data), key).then((m) =>
        this.mux(() => {
          const reply = readMessage(this, this.uid, m, () => {})
          if (reply) {
  ...
  disconnect () {
    ...
    signalingConns.forEach((conn) => {
      if (conn.connected) {
        ...
        log(
          'disconnect but was not connected ',
          logging.BOLD,
          this.name,
          logging.UNBOLD
        )
              ...
              case 'announce':
                if (webrtcConns.size < room.provider.maxConns) {
                  map.setIfUndefined(
                    webrtcConns,
                    data.from,
                    () => new WebrtcConn(this, true, data.from, data.uid, room)
                  )
                  emitPeerChange()
                }
                break
                ...
                if (data.to === peerId) {
                  map
                    .setIfUndefined(
                      ...
                      data.from,
                      () =>
                        new WebrtcConn(this, false, data.from, data.uid, room)
                    )
